       ...
       $curl = curl_init();
        ...
        if($response->status == 'OK'){
            ...
            if($transferRequest->save()){
                ...
                //$data1 =  "New transfer request generated by ".$user->full_name;
                // send messages to users
            $mobile_1 = $user->dialing_code.$user->mobile_no;
            if($mobile_1){
                sendMessage($mobile_1,'mail','patient_transfer_created',null);
            }
           if(isset($user->email)){
                sendMail('mail','patient_transfer_created',null,$user->id,'Transfer Request');
            }
                ...
                if(isset($admin->email)){
                    $existing_data['template'] = 'transfer-request-address';
                    $existing_data['data'] = $transferRequest;
                    sendMail('admin',null,$data1,$admin->id,'Transfer Request',null,$existing_data);
                }
